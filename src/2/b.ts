...
const shapePointsNeeded = (
  ...
  if (
    (opponentShape === 'Rock' && outcomeRequired === 'draw') ||
  ...
  if (
    (opponentShape === 'Rock' && outcomeRequired === 'win') ||
  ...
  if (
    (opponentShape === 'Rock' && outcomeRequired === 'lose') ||
...
totalScore = rounds
  .map((round) => {
    ...
    const opponentDef =
      ABCDefinitions[letters.find((letter) => letter === opponentLetter)];
